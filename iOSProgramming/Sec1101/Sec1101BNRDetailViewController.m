//
//  Sec1101BNRDetailViewController.m
//  iOSProgramming
//
...
@interface Sec1101BNRDetailViewController() <UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIPopoverControllerDelegate>
...
@implementation Sec1101BNRDetailViewController
#pragma mark - 初始化
-(instancetype)initForNewItem:(BOOL)isNew
{
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    self = [storyboard instantiateViewControllerWithIdentifier:@"Sec1101BNRDetailViewController"];
    if (self) {
        if (isNew) {
...
{
    [[Sec1101BNRItemStore sharedStore]removeItem:self.item];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    UIImage *image = [[Sec1101BNRImageStore sharedStore]imageForKey:key];
...
- (IBAction)takePicture:(id)sender {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        Sec1101CameraOverlayView *overlayView = [[Sec1101CameraOverlayView alloc]initWithFrame:self.view.bounds];
...
//    UIImage *image = info[UIImagePickerControllerOriginalImage];
    ...
    [[Sec1101BNRImageStore sharedStore]setImage:image forKey:self.item.itemKey];
...
- (IBAction)clearImage:(id)sender {
    [[Sec1101BNRImageStore sharedStore]deleteImageForKey:self.item.itemKey];
